...
// TEST(CylinderIntersection, Dodecahedron) {
...
//   HalfEdgePolyhedronCylinder<Pt> half_edge;
...
//   std::uniform_real_distribution<double> random_coeffs_r(0.1, 5.0);
...
//         AlignedCylinder({random_coeffs_b(eng), random_coeffs_r(eng)});
...
//     if (first_vertex_on_surface) datum[2] = sqrt<double>(aligned_cylinder.r());
...
//             aligned_cylinder.b() * vertex[1] * vertex[1] - vertex[2];
//         break;
//       }
//       for (auto& vertex : dodeca_local_frame) {
...
TEST(CylinderIntersection, Debug) {
  ...
  // Constructing cells for each subfigure
  std::array<Pt, 8> vertex_list{{Pt(-1.0, -0.2, 0.0),
                                 Pt(1.0, 2.2, 0.0),
                                 Pt(1.0, 0.2, 2.0),
                                 Pt(3.0, 2.6, 2.0),
                                 Pt(-2.0, -0.2, 0.0),
...
// // TEST(CylinderIntersection, TranslatingCubeGradientZ) {
...
// TEST(CylinderIntersection, ProgressiveDistanceSolver) {
//   AlignedParaboloid aligned_paraboloid;
...
// TEST(CylinderIntersection, PtQuad) {
...
// TEST(CylinderIntersection, AutoDiff) {
//   using MyGradientType = ParaboloidGradientLocal;
...
// TEST(CylinderIntersection, AutoDiffM2) {
