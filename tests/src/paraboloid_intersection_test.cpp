...
// #include
// "irl/generic_cutting/paraboloid_intersection/paraboloid_intersection.h"
// #include
// "irl/generic_cutting/paraboloid_intersection/paraboloid_intersection_amr.h"
// #include "irl/geometry/general/normal.h"
// #include "irl/geometry/general/plane.h"
// #include "irl/geometry/half_edge_structures/half_edge_polyhedron_quadratic.h"
// #include
// "irl/geometry/half_edge_structures/segmented_half_edge_polyhedron_quadratic.h"
...
//          3. * std::pow(k, 2.) * M_PI -
//          6. * std::pow(k, 2.) * std::atan(h / std::sqrt(-std::pow(h, 2.) +
//          k)) +
//          6. * std::pow(k, 2.) *
...
//          3. * std::pow(k, 2.) * std::atan(h / std::sqrt(-std::pow(h, 2.) +
//          k)) +
//          3. * std::pow(k, 2.) *
//              std::atan(std::sqrt(-1. + k / std::pow(h, 2.)))) /
//         12.;
...
//          15. * h * std::pow(k, 2.) + 8. * std::pow(-std::pow(h, 2.) +
//          k, 2.5)) /
//         60.;
//   }
//   if ((k - h) > h * h) {
...
//          4. * k * std::sqrt(1. + 4. * k) *
//              std::atan(std::sqrt(-std::pow(h, 2.) + k) / h) +
//          std::atan(4. * h * std::sqrt((-std::pow(h, 2.) + k) / (1. + 4. *
//          k))) + std::atan((h * std::sqrt(1. + 4. * k)) /
//                    std::sqrt(-std::pow(h, 2.) + k)) -
...
//               << std::endl;
//     std::cout << std::setprecision(20)
//               << "M0 unclipped AMR = " << amr_volume_moments.volume() /
//               poly_vol
//               << std::endl;
...
//   using VolumeAndSuface = AddSurfaceOutput<Volume,
...
//         RectangularCuboid::fromBoundingPts(Pt(0.0, 0.0, -k), Pt(h, h, h -
//         k));
...
// //     cube_pt = StoredRectangularCuboid<Pt>::fromOtherPolytope(unit_cell);
// for
// //     (auto& vertex : cube) {
// //       vertex = vertex * h;
...
// //       exact_volume -= (8. * std::pow(h, 3.) * std::sqrt(-std::pow(h, 2.) +
// k)
// //       -
// //                        20. * h * k * std::sqrt(-std::pow(h, 2.) + k) +
// //                        3. * std::pow(k, 2.) * M_PI -
...
// //            6. * h * M_PI + 6. * k * M_PI +
// //            12. * (h - k) * std::atan(h / std::sqrt(-h - std::pow(h, 2.) +
// k))
// //            -
...
// //   std::cout << "Max volume error    = " << max_volume_error << std::endl;
// //   std::cout << "RMS volume error    = " << rms_volume_error << std::endl;
// //   std::cout << "Max gradient error  = " << max_gradient_error <<
// std::endl;
// //   std::cout << "RMS gradient error  = " << rms_gradient_error <<
// std::endl;
// //   std::cout <<
// //   "-------------------------------------------------------------"
// //                "---------------------------------------------------------"
...
// //   EXPECT_NEAR(maximum(max_volume_error, max_gradient_error),
// 0.0, 1.0e-14);
// // }
...
//     const auto cell =
//         StoredRectangularCuboid<Pt>::fromBoundingPts(bottom_corner,
//         top_corner);
...
//         origin, frame, static_cast<double>(alpha),
...
//               << "   Volume double = " << first_moments.volume() <<
//               std::endl;
//     std::cout << " Centroid double = " << first_moments.centroid() <<
//     std::endl; const Quad_t volume_error =
...
//     const auto cell =
//     StoredRectangularCuboid<PtBase<Quad_t>>::fromBoundingPts(
//         bottom_corner, top_corner);
//     // Reference frame and coefficients of paraboloid
//     const auto origin = PtBase<Quad_t>(0, 0, 0);
//     const auto frame = ReferenceFrameBase<Quad_t>(NormalBase<Quad_t>(1, 0,
//     0),
//                                                   NormalBase<Quad_t>(0, 1,
//                                                   0), NormalBase<Quad_t>(0,
//                                                   0, 1));
//     // Create paraboloid object
//     const auto paraboloid = ParaboloidBase<Quad_t>(origin, frame, alpha,
//     beta);
//     // Compute moments of cell clipped by paraboloid
//     const auto first_moments =
//         getVolumeMoments<VolumeMomentsBase<Quad_t>>(cell, paraboloid);
//     std::cout << "     Volume quad = " << first_moments.volume() <<
//     std::endl; std::cout << "   Centroid quad = " << first_moments.centroid()
//     << std::endl; const Quad_t volume_error =
//         fabsq(static_cast<Quad_t>(first_moments.volume()) - exact_volume);
//     const auto m1_error =
//         PtBase<Quad_t>(PtBase<Quad_t>(first_moments.centroid()) - exact_m1);
...
//   double max_error = static_cast<double>(maximum(max_dp_error,
//   max_qp_error)); EXPECT_NEAR(max_error, 0.0, 1.0e-14);
// }
...
//   auto cube =
//   StoredRectangularCuboid<MyPtType>::fromOtherPolytope(unit_cube); auto
//   cube_pt = StoredRectangularCuboid<Pt>::fromOtherPolytope(unit_cube);
...
//   const auto first_moments = getVolumeMoments<MyMomentType>(cube,
//   paraboloid);
...
//   const auto first_moments_A = getVolumeMoments<Volume>(cube_pt,
//   paraboloid_A); const auto first_moments_B =
//   getVolumeMoments<Volume>(cube_pt, paraboloid_B); const auto
//   first_moments_Tx =
...
//   totalTimeElapsed = (double)(processFinishTime - processStartTime) /
//   1000000.;
...
//   auto cube =
//   StoredRectangularCuboid<MyPtType>::fromOtherPolytope(unit_cube);
