...
void print_help(std::string prg_name) {
  ...
  std::cout << "  -f, --input_file <string>              Path to input file "
               "(default: result.csv)\n";
  std::cout << "  -m, --multiplier <int>                 how many time each "
...
void processEntries(const std::string& filename) {
  ...
  std::string line;
  int total = std::count(std::istream_iterator<char>(datafilecount),
  ...
  while (std::getline(datafile, line)) {
    ...
    double b, r, rotation_1, rotation_2, rotation_3, datum_x, datum_y, datum_z,
        is_on_surface, geom, VolumeData, Centroid_x, Centroid_y, Centroid_z;

    ss >> b >> comma >> r >> comma >> rotation_1 >> comma >> rotation_2 >>
        comma >> rotation_3 >> comma >> datum_x >> comma >> datum_y >> comma >>
        datum_z >> comma >> is_on_surface >> comma >> geom >> comma >>
        VolumeData >> comma >> Centroid_x >> comma >> Centroid_y >> comma >>
        Centroid_z;

    file << b << "," << r << "," << rotation_1 << "," << rotation_2 << ","
         << rotation_3 << "," << datum_x << "," << datum_y << "," << datum_z
         << "," << is_on_surface << "," << geom << "," << VolumeData << ","
         << Centroid_x << "," << Centroid_y << "," << Centroid_z << std::endl;
    ...
    IRL::ReferenceFrame frame(IRL::Normal(1.0, 0.0, 0.0),
    ...
    IRL::ReferenceFrame frame0(IRL::Normal(1.0, 0.0, 0.0),
                               IRL::Normal(0.0, 1.0, 0.0),
                               IRL::Normal(0.0, 0.0, 1.0));
    ...
    for (int i = 0; i < multiplier; i++) {
      asm volatile("" ::: "memory");
      result_moments = getVolumeMoments<Volume>(geometriV, cylinder);
    }
    ...
    for (int i = 0; i < multiplier; i++) {
      asm volatile("" ::: "memory");
      result_momentsS = getVolumeMoments<VolumeMoments>(geometriM, cylinder);
    }
    ...
    time_file
        << std::chrono::duration<double, std::micro>(endV - startV).count()
        << ","
        << std::chrono::duration<double, std::micro>(endM - startM).count()
        << std::endl;
    ...
    if (volume_error > threshold ||  //  || Centroid_x_error > threshold ||
                                     //  Centroid_y_error > threshold ||
                                     //  Centroid_z_error > threshold
        volume_errorS > threshold || Centroid_x_errorS > threshold ||
        Centroid_y_errorS > threshold || Centroid_z_errorS > threshold) {
      file2 << "The moment were wrong for this configuration :\n"
            << b << "," << r << "," << rotation_1 << "," << rotation_2 << ","
            << rotation_3 << "," << datum_x << "," << datum_y << "," << datum_z
            << "," << is_on_surface << "," << geom << "\n";
      file2 << "actual   results :\n"
            << volume << "\n"
            <<  // "," << Centroid[0] << "," << Centroid[1] << "," <<
                // Centroid[2] << "\n" <<
          "actual   results :\n"
            << volumeS << "," << CentroidS[0] << "," << CentroidS[1] << ","
            << CentroidS[2] << "\n"
            << "expected results :\n"
            << VolumeData << "," << Centroid_x << "," << Centroid_y << ","
            << Centroid_z << "\n"
            << "errors :\n"
            << volume_error  // << "," << Centroid_x_error << "," <<
                             // Centroid_y_error << "," << Centroid_z_error <<
                             // "\n"
            << "errors :\n"
            << volume_errorS << "," << Centroid_x_errorS << ","
            << Centroid_y_errorS << "," << Centroid_z_errorS << "\n"
            << "\n\n"
            << std::endl;
      wrong_result++;
    }
  ...
  std::cout << "done processing all the cases.\nnumber of wrong result : "
            << wrong_result << std::endl;
  ...
  std::cout << "\n\ntime taken to do " << (total) << " cases : \n"
            << "    Only Volume :" << time_Volume.count() / 1e6
            << " s or an average of "
            << time_Volume.count() / (total * multiplier) << " µs per case\n"
            << "    with Moment :" << time_Moment.count() / 1e6
            << " s or an average of "
            << time_Moment.count() / (total * multiplier) << " µs per case"
            << std::endl;

  std::cout << "sanity check : i = " << i << ", total = " << total * multiplier
            << std::endl;
}
