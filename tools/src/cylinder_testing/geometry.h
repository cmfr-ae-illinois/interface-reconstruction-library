...
std::pair<IRL::StoredGeneralPolyhedron<IRL::Pt, 20>,
          IRL::PolyhedronConnectivity*>
getGeometry(const int a_geometry, bool re_scale) {
  ...
  if (a_geometry == 1) {
    if (re_scale) {
      ...
      vertices = std::vector<IRL::Pt>{
          {IRL::Pt(0.5, 0.0, -0.5), IRL::Pt(-0.5, 0.5, -0.5),
           IRL::Pt(-0.5, -0.5, -0.5), IRL::Pt(0.0, 0.0, 0.5)}};
      std::array<std::array<IRL::UnsignedIndex_t, 3>, 4> face_mapping{
          {{0, 1, 3}, {1, 2, 3}, {1, 0, 2}, {0, 3, 2}}};
      connectivity = new IRL::PolyhedronConnectivity(face_mapping);
    }
  ...
  auto polyhedron =
      IRL::StoredGeneralPolyhedron<IRL::Pt, 20>(vertices, connectivity);
