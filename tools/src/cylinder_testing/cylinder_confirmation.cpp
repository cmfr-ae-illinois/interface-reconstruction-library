...
void processEntries(const std::string& filename) {
  ...
  while (std::getline(datafile, line)) {
    ...
    result_moments = getVolumeMoments<VolumeMoments>(geometri, cylinder);
    auto result_moments_with_surface = getVolumeMoments<
    ...
    if (volume_error > threshold || Centroid_x_error > threshold ||
        Centroid_y_error > threshold || Centroid_z_error > threshold ||
      ...
      file2 << "The moment were wrong for this configuration :\n"
            ...
            << rotation_3 << "," << datum_x << "," << datum_y << "," << datum_z
            << "," << is_on_surface << "," << geom << "\n";
      file2 << "actual   results (no surface) :\n"
            << volume << "," << Centroid[0] << "," << Centroid[1] << ","
            << Centroid[2] << "\n"
            << "actual   results :\n"
  ...
  std::cout << "\n\ntime taken to do " << (2 * total)
            << " cases : " << total_time.count() << " s or an average of "
            << total_time.count() * 1e6 / (2.0 * total) << " µs per case"
            << std::endl;
