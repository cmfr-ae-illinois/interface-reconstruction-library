...
// DEBUG_CYL_IRL add general log in the standard output about cylinder
// intersection
...
#include "tools/src/cylinder_testing/geometry.h"
...
void print_help(const char* program_name) {
  ...
  std::cout << "  " << program_name
            << " [OPTIONS] -- b r datum_x datum_y datum_z rotation_x "
               "rotation_y rotation_z [first_vertex_on_surface]\n";
  std::cout << "  " << program_name
            << " [OPTIONS] -- "
               "b,r,rotation_x,rotation_y,rotation_z,datum_x,datum_y,datum_z[,"
               "first_vertex_on_surface]\n";
  ...
  std::cout << "  -H, --help                                         Show this "
               "help message\n";
  std::cout << "  -l, --AMR_level <int> / --max_AMR_level <int>      Set the "
               "maximum AMR level (default: 10)\n";
  std::cout << "  -f, --file <string>                                Input "
               "file path\n";
  std::cout << "  -r, --random_position[=<seed>]                     Generate "
               "random coordinates; optional seed\n";
  std::cout
      << "  -L, --triangluation_Level                          Set the max "
         "distance used for triangulating the output surface (default : 0.1)\n";
  std::cout << "      --no_surface_output                            Disable "
               "surface output\n";
  std::cout << "      --first_vertex_on_surface                      Place "
               "first vertex on surface\n";
  std::cout << "      --no_rescale                                   Disable "
               "rescaling\n";
  std::cout << "      --force_rescale                                Force "
               "rescaling\n";
  std::cout << "      --old_format                                   Use old "
               "input format (remove rotation_z from input)\n";
  std::cout << "      --cube                                         Use cube "
               "polyhedron\n";
  std::cout
      << "      --old_cube                                     Use old cube\n";
  std::cout << "      --tet                                          Use "
               "tetrahedron\n";
  std::cout << "      --dodecahedron                                 Use "
               "dodecahedron\n";
  std::cout << "      --cube_hole                                    Use cube "
               "with a hole\n";
  std::cout << "      --cube_hole_convex                             Use "
               "convex cube with a hole\n";
  std::cout << "      --bunny                                        Use bunny "
               "mesh\n";
}

template <class HalfEdgeType, class SegmentedHalfEdge, class CuttingType>
void procede_case(Cylinder a_cylinder, CuttingType irl_cutting,
                  SegmentedHalfEdge a_segmented_half_edge,
                  HalfEdgeType a_half_edge, int max_amr_level,
                  float tri_level) {
  ...
  auto temp_moments = getVolumeMoments<VolumeMoments>(irl_cutting, a_cylinder);
  auto start_amr = std::chrono::high_resolution_clock::now();
  std::string output_file =
      no_surface_output ? IRL::no_amr_output : "_test_config_cube";
#ifdef DEBUG_CYL_IRL
  std::cout << "computing AMR" << std::endl;
#endif
  auto amr_moments = intersectPolyhedronWithCylinderAMR<VolumeMoments>(
      &(a_segmented_half_edge), &(a_half_edge), a_cylinder.getAlignedCylinder(),
      max_amr_level, output_file);
  auto end_amr = std::chrono::high_resolution_clock::now();
  std::cout << "the computed volume (with surface) is :"
            << std::setprecision(20)
            << temp_surface_and_moments.getMoments().volume().volume()
            << std::endl;
  std::cout << "the computed volume (w/o  surface) is :"
            << std::setprecision(20) << temp_moments.volume().volume()
            << std::endl;
  std::cout << "the amr      volume                is :"
            << amr_moments.volume().volume() << std::endl;
  ...
  std::cout << "errors :\n"
            << volume_error << "," << error_centroid[0] << ","
            << error_centroid[1] << "," << error_centroid[2] << std::endl;
  std::cout << "errors (surface ):\n"
            << volume_errorS << "," << error_centroidS[0] << ","
            << error_centroidS[1] << "," << error_centroidS[2] << std::endl;
...
struct DataEntry {
  double b, r, rotation_1, rotation_2, rotation_3, datum_x, datum_y, datum_z,
      is_on_surface, geometry;
};
...
void read_file(char* filepath, int max_amr_level, float tri_level) {
  ...
  while (std::getline(file, line)) {
    ...
    ss >> entry.b >> comma >> entry.r >> comma >> entry.rotation_1 >> comma >>
        entry.rotation_2 >> comma >> entry.rotation_3 >> comma >>
        entry.datum_x >> comma >> entry.datum_y >> comma >> entry.datum_z >>
        comma >> entry.is_on_surface >> comma >> entry.geometry;
  ...
  for (auto data : datas) {
    ...
    if (old_polyhedron) {
      RectangularCuboid cube = RectangularCuboid::fromBoundingPts(
          Pt(0.0, 0.0, 0.0), Pt(1.0, 1.0, 1.0));
      RectangularCuboid amr_cube = RectangularCuboid::fromBoundingPts(
          Pt(0.0, 0.0, 0.0), Pt(1.0, 1.0, 1.0));
      ...
      Cylinder cylinder(cyl_datum, frame, aligned_cylinder.b(),
                        aligned_cylinder.r());
...
int main(int argc, char* argv[]) {
  ...
  if (filepath == nullptr && (optind != argc - 1 && optind != argc - 8) &&
      !random_coords) {
    std::cerr
        << "Usage: " << argv[0]
        << " -- b r datum_x datum_y datum_z rotation_x rotation_y rotation_z\n";
    std::cerr << "or   : " << argv[0]
              << " -- "
                 "b,r,rotation_x,rotation_y,rotation_z,datum_x,datum_y,datum_z["
                 ",first_vertex_on_surface[,geometry]]\n";
  ...
  if (filepath == nullptr) {
    ...
    if (random_coords) {
      std::cout << "using random parameters initialized with seed : " << seed
                << std::endl;
      ...
      if (optind == argc - 1) {
        ...
        double dbl_fvos;
        double geom;
        ss >> b >> comma >> r >> comma >> rotation_x >> comma >> rotation_y >>
            comma >> rotation_z >> comma >> datum_x >> comma >> datum_y >>
            comma >> datum_z >> comma >> dbl_fvos >> comma >> geom;
    ...
    std::cout << "b : " << b << std::endl
              ...
              << "is the first vertex on surface ? : "
              << first_vertex_on_surface << std::endl
