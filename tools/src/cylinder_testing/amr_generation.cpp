...
static const std::array<double, 9> gratted_b({0.9, 1.0, 16.0 / 9.0, 2.0,
                                              9.0 / 4.0, 4, -0.75, -1.0,
                                              -1.25});
static const std::array<double, 5> gratted_r({0.0625, 0.25, 0.5, 9.0 / 16.0,
                                              1.0});
static const std::array<double, 5> gratted_rotation({-M_PI, -M_PI / 2.0, 0.0,
                                                     M_PI / 2.0, M_PI});
static const std::array<double, 5> gratted_translation({-0.5, -0.25, 0.0, 0.25,
                                                        0.5});
...
std::array<double, 8> get_config(int i) {
  ...
  return {gratted_b[b_index],
          gratted_r[r_index],
          gratted_rotation[rotation_index_1],
          gratted_rotation[rotation_index_2],
          gratted_rotation[rotation_index_3],
          gratted_translation[translation_index_1],
          gratted_translation[translation_index_2],
          gratted_translation[translation_index_3]};
}
...
void print_help(std::string prg_name) {
  ...
  std::cout << "  -n, --number_of_configuration <int>    Number of "
               "configurations (default: 1000)\n";
  std::cout
      << "  -l, --AMR_level <int>                  AMR level (default: 15)\n";
  std::cout << "  -s, --seed <int>                       Random seed (default: "
               "current time)\n";
  std::cout << "  -o, --file_name <string>               Output file name "
               "(default: result.csv)\n";
  std::cout << "  -e, --elliptic                         Use elliptic bounds "
               "(min_b=0, max_b=10)\n";
  std::cout << "  -h, --hyperbolic                       Use hyperbolic bounds "
               "(min_b=-10, max_b=0)\n";
  std::cout << "  -b, --both                             Use full bounds "
               "(min_b=-10, max_b=10)\n";
  std::cout << "  -g, --graded_gen                       Use graded generation "
               "mode\n";
  std::cout << "      --first_vertex_on_surface          Start with first "
               "vertex on surface\n";
  std::cout << "      --no_rescale                       Disable rescaling\n";
  std::cout << "      --force_rescale                    Force rescaling\n";
  std::cout << "      --cube                             Use cube polyhedron "
               "(default)\n";
  std::cout << "      --old_cube                         Use old cube (center "
               "is 0.5 0.5 0.5 and is not rescaled)\n";
  std::cout << "      --tet                              Use tetrahedron\n";
  std::cout << "      --dodecahedron                     Use dodecahedron\n";
  std::cout << "      --cube_hole                        Use cube with hole\n";
  std::cout
      << "      --cube_hole_convex                 Use convex cube with hole\n";
...
int main(int argc, char* argv[]) {
  ...
  while (1) {
    int result =
        getopt_long(argc, argv, "l:n:s:ehbo:gH", long_options, &long_id);
    if (result == -1) {
      break;
    }
    switch (result) {
...
#pragma omp single
    {
      ...
      std::cout << "running with " << omp_get_max_threads()
                << " threads (seed is :" << seed
                << ") polygon type : " << polytype << std::endl;
      std::ofstream file(filename);

      // Write the CSV header
      file << "b,r," << "rotation_1,rotation_2,rotation_3,"
           << "datum_x,datum_y,datum_z," << "is_on_surface," << "geometry,"
           << "Volume," << "Centroid_x,Centroid_y,Centroid_z" << std::endl;
      file.close();
    }
...
#pragma omp barrier
    ...
    std::uniform_real_distribution<double> random_rotation(-M_PI, M_PI);
...
#pragma omp for nowait schedule(dynamic)
    for (int i = 0; i < Ntests; i++) {
      ...
      auto aligned_cylinder = AlignedCylinder({b, r});
...
#pragma omp critical
      {
        ...
        file << amr_Centroid[0] << "," << amr_Centroid[1] << ","
             << amr_Centroid[2] << std::endl;
        file.close();
      }
...
#pragma omp critical
    {
      std::cout << "Results written to " << filename << ", time taken to do "
                << count << "cases : " << j << ":" << h << ":" << m << ":" << s
                << std::endl;
    }
  }
