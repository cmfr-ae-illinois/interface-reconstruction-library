...
struct LagrangianRemap {
  static void TransportMoments(const SepUnionMultiFab& a_interface_with_ghost,
    ...
    for (MFIter mfi(a_interface_with_ghost, TilingIfNotGPU()); mfi.isValid();
         ++mfi) {
      // Get internal box and box with ghosts
      const Box& bx = mfi.tilebox();
      const Box& grown_bx = mfi.growntilebox();

      // Get arrays for velocity, interface, band_id and moments
      Array4<Real const> velx = a_facevel[0].const_array(mfi);
      Array4<Real const> vely = a_facevel[1].const_array(mfi);
      Array4<Real const> velz = a_facevel[2].const_array(mfi);
      Array4<IRL::SeparatorUnion const> interface_array =
          a_interface_with_ghost.const_array(mfi);
      Array4<Real const> band_id_array = a_band_id.const_array(mfi);
      ...
      amrex::ParallelFor(bx, [=] AMREX_GPU_DEVICE(int i, int j, int k) {
        if (band_id_array(i, j, k) != 0.0) {
          ...
          for (int n = 0; n < 8; ++n) {
            preimage[n] = ProjectVertex(cell[n], -a_dt, velx, vely, velz,
                                        grown_bx, a_geom);
          }
          ...
          for (int f = 0; f < 6; f++) {
            ...
            flux[8] =
                ProjectVertex(0.25 * (flux[0] + flux[1] + flux[2] + flux[3]),
                              -a_dt, velx, vely, velz, grown_bx, a_geom);
          ...
          for (int ii = ilo; ii <= ihi; ii++) {
            for (int jj = jlo; jj <= jhi; jj++) {
              for (int kk = klo; kk <= khi; kk++) {
                ...
                centroid = ProjectVertex(centroid, a_dt, velx, vely, velz,
                                         grown_bx, a_geom);
...
// struct LagrangianRemap {
//   static void TransportMoments(const SepUnionMultiFab&
//   a_interface_with_ghost,
//                                const Array<MultiFab, AMREX_SPACEDIM>&
//                                a_facevel, const MultiFab& a_band_id,
...
//           // Compute the back projected cell corners
//           for (int n = 0; n < 8; ++n) {
//             preimage[n] = ProjectVertex(cell[n], -a_dt, a_time);
//           }
//           // Compute soleinoidal flux volumes
//           const IRL::Pt xlo_face(x, y + 0.5 * dx[1], z + 0.5 * dx[2]);
//           const IRL::Pt xhi_face(x + dx[0], y + 0.5 * dx[1], z + 0.5 *
//           dx[2]);

//           const IRL::Pt ylo_face(x + 0.5 * dx[0], y, z + 0.5 * dx[2]);
//           const IRL::Pt yhi_face(x + 0.5 * dx[0], y + dx[1], z + 0.5 *
//           dx[2]);

//           const IRL::Pt zlo_face(x + 0.5 * dx[0], y + 0.5 * dx[1], z);
//           const IRL::Pt zhi_face(x + 0.5 * dx[0], y + 0.5 * dx[1], z +
//           dx[2]);

//           const auto u_xlo = GetDeformationTestCaseVelocity(xlo_face,
//           a_time); const auto u_xhi =
//           GetDeformationTestCaseVelocity(xhi_face, a_time);

//           const auto u_ylo = GetDeformationTestCaseVelocity(ylo_face,
//           a_time); const auto u_yhi =
//           GetDeformationTestCaseVelocity(yhi_face, a_time);

//           const auto u_zlo = GetDeformationTestCaseVelocity(zlo_face,
//           a_time); const auto u_zhi =
//           GetDeformationTestCaseVelocity(zhi_face, a_time);
...
//                 0.25 * (flux[0] + flux[1] + flux[2] + flux[3]), -a_dt,
//                 a_time);
//             flux.adjustCapToMatchVolume(flux_volumes[f]);
//             preimage[face_center_id_table[f]] = flux[8];
//           }
...
//           // Compute bounding box of preimage
//           Real xlo = preimage[0][0], ylo = preimage[0][1], zlo =
//           preimage[0][2]; Real xhi = preimage[0][0], yhi = preimage[0][1],
//           zhi = preimage[0][2]; for (int n = 1; n < 14; n++) {
//             xlo = preimage[n][0] < xlo ? preimage[n][0] : xlo;
//             ylo = preimage[n][1] < ylo ? preimage[n][1] : ylo;
//             zlo = preimage[n][2] < zlo ? preimage[n][2] : zlo;
//             xhi = preimage[n][0] > xhi ? preimage[n][0] : xhi;
...
//           }
//           int ilo =
//               static_cast<int>(amrex::Math::floor((xlo - problo[0]) /
//               dx[0]));
//           int jlo =
//               static_cast<int>(amrex::Math::floor((ylo - problo[1]) /
//               dx[1]));
//           int klo =
//               static_cast<int>(amrex::Math::floor((zlo - problo[2]) /
//               dx[2]));
//           int ihi =
//               static_cast<int>(amrex::Math::floor((xhi - problo[0]) /
//               dx[0]));
//           int jhi =
//               static_cast<int>(amrex::Math::floor((yhi - problo[1]) /
//               dx[1]));
//           int khi =
//               static_cast<int>(amrex::Math::floor((zhi - problo[2]) /
//               dx[2]));
...
//                 centroid[2] = std::max(centroid[2], zloc);
//                 centroid[2] = std::min(centroid[2], zloc + dx[2]);
//                 centroid = ProjectVertex(centroid, a_dt, a_time - a_dt);
//                 moments_array(i, j, k, 0) += m0;
//                 moments_array(i, j, k, 1) += m0 * centroid[0];
//                 moments_array(i, j, k, 2) += m0 * centroid[1];
//                 moments_array(i, j, k, 3) += m0 * centroid[2];
