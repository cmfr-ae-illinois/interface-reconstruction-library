    ...
    for (MFIter mfi(refined_level_vf, TilingIfNotGPU()); mfi.isValid(); ++mfi) {
      ...
      ParallelFor(refined_box,
                  [=] AMREX_GPU_DEVICE(int i, int j, int k) noexcept {
                    ...
                    Real vf = lev_mom(ic, jc, kc, 0) / lev_vol;
...
void AmrCoreAdv::BuildUniformFinestMoments(
  ...
  for (MFIter mfi(a_uniform_moments, TilingIfNotGPU()); mfi.isValid(); ++mfi) {
    ...
    ParallelFor(box, [=] AMREX_GPU_DEVICE(int i, int j, int k) noexcept {
      ...
      if (vf > IRL::global_constants::VF_HIGH) {
        mom_arr(i, j, k, 0) = fine_vol;
        if (transport_m1) {
          mom_arr(i, j, k, 1) = fine_vol * x;
          mom_arr(i, j, k, 2) = fine_vol * y;
          mom_arr(i, j, k, 3) = fine_vol * z;
          // Gas is empty
          mom_arr(i, j, k, 4) = Real(0.0);
          mom_arr(i, j, k, 5) = Real(0.0);
          mom_arr(i, j, k, 6) = Real(0.0);
        }
        if (transport_m2) {
          // Liquid Mxx
          mom_arr(i, j, k, 7) =
          ...
          // Liquid Mxy
          mom_arr(i, j, k, 8) = fine_vol * x * y;
          // Liquid Mxz
          mom_arr(i, j, k, 9) = fine_vol * x * z;
          // Liquid Myy
          mom_arr(i, j, k, 10) =
              fine_vol * (fine_dx[1] * fine_dx[1] / Real(12.0) + y * y);
          // Liquid Myz
          mom_arr(i, j, k, 11) = fine_vol * y * z;
          // Liquid Mzz
          mom_arr(i, j, k, 12) =
              fine_vol * (fine_dx[2] * fine_dx[2] / Real(12.0) + z * z);
          // Gas is empty
          mom_arr(i, j, k, 13) = Real(0.0);
          mom_arr(i, j, k, 14) = Real(0.0);
          mom_arr(i, j, k, 15) = Real(0.0);
          mom_arr(i, j, k, 16) = Real(0.0);
          mom_arr(i, j, k, 17) = Real(0.0);
          mom_arr(i, j, k, 18) = Real(0.0);
        }

        // gas cell
      } else if (vf < IRL::global_constants::VF_LOW) {
        mom_arr(i, j, k, 0) = Real(0.0);
        if (transport_m1) {
          // Liquid is empty
          mom_arr(i, j, k, 1) = Real(0.0);
          mom_arr(i, j, k, 2) = Real(0.0);
          mom_arr(i, j, k, 3) = Real(0.0);
          // Gas occupies complete cell
          mom_arr(i, j, k, 4) = fine_vol * x;
          mom_arr(i, j, k, 5) = fine_vol * y;
          mom_arr(i, j, k, 6) = fine_vol * z;
        }

        if (transport_m2) {
          // Liquid is empty
          mom_arr(i, j, k, 7) = Real(0.0);
          mom_arr(i, j, k, 8) = Real(0.0);
          mom_arr(i, j, k, 9) = Real(0.0);
          mom_arr(i, j, k, 10) = Real(0.0);
          mom_arr(i, j, k, 11) = Real(0.0);
          mom_arr(i, j, k, 12) = Real(0.0);
          // Gas Mxx
          mom_arr(i, j, k, 13) =
              fine_vol * (fine_dx[0] * fine_dx[0] / Real(12.0) + x * x);
          // Gas Mxy
          mom_arr(i, j, k, 14) = fine_vol * x * y;
          // Gas Mxz
          mom_arr(i, j, k, 15) = fine_vol * x * z;
          // Gas Myy
          mom_arr(i, j, k, 16) =
              fine_vol * (fine_dx[1] * fine_dx[1] / Real(12.0) + y * y);
          // Gas Myz
          mom_arr(i, j, k, 17) = fine_vol * y * z;
          // Gas Mzz
          mom_arr(i, j, k, 18) =
              fine_vol * (fine_dx[2] * fine_dx[2] / Real(12.0) + z * z);
        }
  ...
  a_uniform_moments.ParallelCopy(moments_new[finest], 0, 0, ncomp, 0, 0,
                                 Geom(finest).periodicity());
}
