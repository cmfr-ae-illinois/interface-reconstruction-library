...
void VTKOutput::writeVTKInterface(
    ...
    std::vector<double>& a_polygon_data, std::vector<double>& a_paraboloid_data,
    std::vector<double>& a_cylinder_data, const bool a_print_info) {
  ...
  for (int i = 0; i < a_paraboloids.size(); ++i) {
    ...
    for (int j = ntriangles; j < surface_output.nBezierTriangles(); j++) {
      data.push_back(a_paraboloid_data[i]);
    }
  }
  ...
  for (int i = 0; i < a_polygons.size(); i++) {
    data.push_back(a_polygon_data[i]);
  }
  surface_output.write(surface_file_name);
  const int ncells =
      surface_output.nBezierTriangles() + surface_output.nPolygons();
  surface_output.addCellData(surface_file_name, "data", data);
