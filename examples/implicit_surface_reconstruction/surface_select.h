...
inline SurfaceVariant makeSurface(const std::string& name, BasicMesh& mesh) {
  if (name == "sphere") {
    mesh.setCellBoundaries(IRL::Pt(-0.22, -0.22, -0.22),
                           IRL::Pt(0.22, 0.22, 0.22));
    return SphereVariant(0., 0., 0., 0.15);
  } else if (name == "ellipsoid") {
    mesh.setCellBoundaries(IRL::Pt(-0.35, -0.35, -0.35),
                           IRL::Pt(0.35, 0.35, 0.35));
    return EllipsoidVariant(0., 0., 0., 0.30, 0.15, 0.10);
  } else if (name == "genus") {
    mesh.setCellBoundaries(IRL::Pt(-2, -2, -2), IRL::Pt(2, 2, 2));
    return GenusVariant{};
  } else if (name == "orthocircle") {
    mesh.setCellBoundaries(IRL::Pt(-1.9, -1.9, -1.9), IRL::Pt(1.9, 1.9, 1.9));
