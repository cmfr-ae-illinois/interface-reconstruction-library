...
namespace IRL {
// ============== Wu Class Functions
inline double Wu::computeR(Pt xi, Pt x_eval) {
  Pt dx = x_eval - xi;
  return std::sqrt(dx[0] * dx[0] + dx[1] * dx[1] + dx[2] * dx[2]);
}

inline double Wu::eval(double r, double delta) {
  double rhat = r / delta;
  if (rhat <= 1.0) {
...
inline Eigen::Vector3d Wu::getGradient(const Pt& xi, const double& delta,
                                       const Pt& x_eval) {
  // First, get r
  double r = Wu::computeR(xi, x_eval);
  if (r > delta) {
...
inline Eigen::Matrix3d Wu::getHessian(const Pt& xi, const double& delta,
                                      const Pt& x_eval) {
  // First, get r
  double r = Wu::computeR(xi, x_eval);
  if (r > delta) {
...
// Evaluate 1
inline void Wu::evaluate(const Pt& xi, const double& delta, const Pt& x_eval,
                         double* retVal) {
  // First, get r
  double r = Wu::computeR(xi, x_eval);
  // Next Calculate F, the function value
  double F = Wu::eval(r, delta);
  // Return
  *retVal = F;
}
