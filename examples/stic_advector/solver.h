...
int runSimulation(const double a_dt, const double a_end_time,
  ...
  Data<double> old_liquid_volume_fraction(&cc_mesh);
  ...
  while (simulation_time < a_end_time) {
    ...
    old_liquid_volume_fraction = liquid_volume_fraction;
    advectVOF("SemiLagrangianCorrected", time_step_to_use, velU, velV, velW,
    ...
    getReconstruction(liquid_volume_fraction, liquid_centroid, gas_centroid,
                      link_localized_separators, time_step_to_use, velU, velV,
                      velW, &interface);
