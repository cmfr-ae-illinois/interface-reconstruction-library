...
#include "examples/stic_advector/basic_mesh.h"
#include "examples/stic_advector/data.h"
...
void writeOutVisualization(const int a_iteration,
  ...
  fprintf(viz_file, "<VTKFile type=\"RectilinearGrid\">\n");
  fprintf(viz_file, "<RectilinearGrid WholeExtent=\"%d %d %d %d %d %d\">\n",
          mesh.imin(), mesh.imax() + 1, mesh.jmin(), mesh.jmax() + 1,
          mesh.kmin(), mesh.kmax() + 1);
  fprintf(viz_file, "<Piece Extent=\"%d %d %d %d %d %d\">\n", mesh.imin(),
          mesh.imax() + 1, mesh.jmin(), mesh.jmax() + 1, mesh.kmin(),
          mesh.kmax() + 1);

  fprintf(viz_file, "<Coordinates>\n");
  fprintf(viz_file,
          "<DataArray type=\"Float64\" NumberOfComponents=\"1\" "
          "format=\"ascii\">\n");
  for (int i = mesh.imin(); i <= mesh.imax() + 1; ++i) {
    fprintf(viz_file, "%15.8E ", static_cast<double>(mesh.x(i)));
  }
  fprintf(viz_file, "\n</DataArray>\n");
  fprintf(viz_file,
          "<DataArray type=\"Float64\" NumberOfComponents=\"1\" "
          "format=\"ascii\">\n");
  for (int i = mesh.jmin(); i <= mesh.jmax() + 1; ++i) {
    fprintf(viz_file, "%15.8E ", static_cast<double>(mesh.y(i)));
  }
  fprintf(viz_file, "\n</DataArray>\n");
  ...
  fprintf(viz_file,
          "<DataArray type=\"Float64\" NumberOfComponents=\"1\" "
          "format=\"ascii\">\n");
  for (int i = mesh.kmin(); i <= mesh.kmax() + 1; ++i) {
    fprintf(viz_file, "%15.8E ", static_cast<double>(mesh.z(i)));
  }
  fprintf(viz_file, "\n</DataArray>\n");

  fprintf(viz_file, "</Coordinates>\n");

  fprintf(viz_file, "<PointData>\n</PointData>\n");

  fprintf(viz_file, "<CellData Scalars=\"1\" >\n");
  fprintf(viz_file,
          "<DataArray type=\"Float64\" Name=\"VolumeFraction\" "
          "NumberOfComponents=\"1\" "
          "format=\"ascii\">\n");
  for (int k = mesh.kmin(); k <= mesh.kmax(); ++k) {
    for (int j = mesh.jmin(); j <= mesh.jmax(); ++j) {
      for (int i = mesh.imin(); i <= mesh.imax(); ++i) {
        fprintf(viz_file, "%15.8E ", a_liquid_volume_fraction(i, j, k));
      }
    }
  }
  fprintf(viz_file, "\n</DataArray>\n");
  fprintf(viz_file, "</CellData>\n");
  fprintf(viz_file, "</Piece>\n</RectilinearGrid>\n</VTKFile>\n");
  fclose(viz_file);
}
