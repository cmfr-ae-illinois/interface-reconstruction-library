...
#include "examples/2d_advector_debug/deformation_2d.h"
#include "examples/2d_advector_debug/irl2d.h"
#include "examples/2d_advector_debug/oscillation_2d.h"
...
void FullLag::advectVOF(const std::string& a_simulation_type,
  ...
  //   for (int j = mesh.jmin(); j <= mesh.jmax(); ++j) {
  //     // initializing
  //     triangle_liq_mom(i,j) = std::vector<IRL2D::Moments>(num_triangles, IRL2D::Moments()); //IRL2D::InitializeTriangleMoments(num_triangles);
    ...
    for (int j = mesh.jmin(); j <= mesh.jmax(); ++j) {
      ...
      if (band(i, j) > 0) {
        ...
        for (int m = 0; m < 2; m++) {
          ...
          // Transport of moment 1 (implement mapping here)
          auto x0_k1 = M1 / IRL::safelyEpsilon(M0);
          const auto u0_k1 = getExactVelocity2D(a_time, x0_k1);
          const auto gradu0_k1 = getExactGradient2D(a_time, x0_k1);
          const auto x0_k2 = x0_k1 + a_dt * u0_k1 * 0.5;
