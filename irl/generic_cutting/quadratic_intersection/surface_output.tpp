...
inline void addEllipseToSurfaceOutput(
  ...
  const auto aligned_cylinder =
      AlignedCylinderBase<double>(a_aligned_cylinder);
  const auto face_plane = PlaneBase<double>(
      NormalBase<double>(static_cast<double>(a_face_plane.normal()[0]),
                         static_cast<double>(a_face_plane.normal()[1]),
                         static_cast<double>(a_face_plane.normal()[2])),
  ...
  assert(std::fabs(face_plane.normal()[0]) > DBL_EPSILON);
  const double r = aligned_cylinder.r();
  const double beta = aligned_cylinder.b();
  const double a = -face_plane.normal()[1] / face_plane.normal()[0];
  const double b = -face_plane.normal()[2] / face_plane.normal()[0];
  const double c = face_plane.distance() / face_plane.normal()[0];
  static constexpr UnsignedIndex_t nSplit = 3;  // Minimum acceptable is 3
  std::array<Pt, nSplit> points;
  const double angle_interval = 2.0 * M_PI / static_cast<double>(nSplit);
  const double normal_invert = std::copysign(1.0, face_plane.normal()[0]);
  const double invert =
      beta < 0.0 ? normal_invert : -normal_invert;
  for (UnsignedIndex_t i = 0; i < nSplit; ++i) {
    const double theta = invert * static_cast<double>(i) * angle_interval;
    points[i] = Pt(0.0, sqrt(r / beta) * cos(theta),
                   sqrt(r) * sin(theta));
    points[i][0] = a * points[i][1] + b * points[i][2] + c;
  }
  auto gradF_0 = getCylinderSurfaceNormal(aligned_cylinder, points[0]);
  Normal tangent_0 = crossProduct(face_plane.normal(), gradF_0);
  tangent_0.normalize();
  Normal start_tangent = tangent_0;
  for (UnsignedIndex_t i = 0; i < nSplit - 1; ++i) {
    auto gradF_end =
        getCylinderSurfaceNormal(aligned_cylinder, points[i + 1]);
    Normal end_tangent = crossProduct(face_plane.normal(), gradF_end);
    end_tangent.normalize();
    if (dotProduct(start_tangent, points[i + 1] - points[i]) < 0.0) {
      start_tangent *= -1.0;
