...
namespace IRL {
    ...
    void PUImplicitSurface::evaluate(Pt& x,std::tuple<double,Eigen::Vector3d,Eigen::Matrix3d>* retVal) {
        ...
        for(int i =0;i < centroids.size(); ++i) { // Loop over separators
            ...
            PUImplicitSurface::implicitSeparator(x,centroids[i],&separators[i],&separatorRet);
    ...
    void PUImplicitSurface::evaluate(Pt& x,std::pair<double,Eigen::Vector3d>* retVal) {
        double weight_sum = 0.0;
        double F_sum = 0.0;
        Eigen::Vector3d grad_weight_sum = Eigen::Vector3d::Zero();
        Eigen::Vector3d grad_product_sum = Eigen::Vector3d::Zero();
