...
#include "irl/c_interface/interface_reconstruction_methods/c_volume_fraction_matching.h"
#include "irl/data_structures/raw_pointer_wrapper.h"
...
void c_matchGroupVolumeFraction_Tet_PlanarSepPathGroup_Default(
  ...
  assert(a_reconstruction->obj_ptr != nullptr);
  assert(*a_size >= 0);
  auto wrapped_vf = IRL::wrapRawPointer(
      a_volume_fraction, static_cast<IRL::UnsignedIndex_t>(*a_size));
  IRL::setGroupDistanceToMatchVolumeFraction(*a_cell->obj_ptr, wrapped_vf,
                                             a_reconstruction->obj_ptr);
}
