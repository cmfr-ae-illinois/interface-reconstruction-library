...
inline RationalBezierArcBase<ScalarType>::RationalBezierArcBase(
  ...
  if (fabs(n_cross_t0 * a_end_tangent) < ONEHUNDRED * EPSILON) {
    ...
    if (L < DISTANCE_EPSILON * DISTANCE_EPSILON) {
      weight_m = DEFAULT_WEIGHT;
    } else if (fabs(a_endpoint_curvature) < DISTANCE_EPSILON) {
      weight_m = DEFAULT_WEIGHT;
    } else {
      const ScalarType cross_mag = magnitude(
          crossProduct(a_end_pt - a_start_pt, control_point_m - a_end_pt));
      const ScalarType A = HALF * cross_mag;
      const ScalarType D = sqrt(L);

      weight_m = sqrt(A / (a_endpoint_curvature * D * D * D));
    }
  }
  // Clip weight to avoid variable overflows
  if (a_endpoint_curvature < ZERO) {
    if (weight_m >= ZERO) {
      weight_m = minimum(weight_m, ScalarType(1.0 / DBL_EPSILON));
      weight_m = maximum(weight_m, ONE);
    }
